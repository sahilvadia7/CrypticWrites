...
const ManageCreators = () => {
  ...
  const addOrUpdateCreator = () => {
    if (!newCreator.role || !newCreator.email) {
      setError("Both role and email are required");
      return;
    }
    const apiUrl = editMode
      ? `http://localhost:8080/api/user/updateUser/${newCreator.id}`
        ...
        setCreators(
          editMode
            ? creators.map((creator) =>
                creator.id === data.id ? data : creator,
              )
            : [...creators, data],
        );
  ...
  const deleteCreator = (id) => {
    fetch(`http://localhost:8080/api/deleteUser/${id}`, { method: "DELETE" })
      .then(() => {
    ...
    <div className="admin-container">
      ...
      <div>
        <input
          ...
          onChange={(e) =>
            setNewCreator({ ...newCreator, role: e.target.value })
          }
        />
        <input
          type="email"
